...
def playfair(key, remaining, encrypting=True):
    ...
    while remaining != "":
        ...
        shift = (
            1 if encrypting else -1
        )  # for same column/row; if encrypting shift +1, else shift -1
        if c1[0] == c2[0]:  # same row
            cipher = (
                cipher
                + table[pos(c1[0], (c1[1] + shift) % 5)]
                + table[pos(c2[0], (c2[1] + shift) % 5)]
            )
        elif c1[1] == c2[1]:  # same column
            cipher = (
                cipher
                + table[pos((c1[0] + shift) % 5, c1[1])]
                + table[pos((c2[0] + shift) % 5, c2[1])]
            )
        else:
