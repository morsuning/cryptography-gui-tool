...
class Event(UiMainWindow, QObject):
    ...
    def setup_mapper(self):
        self.cipher_map = {
            "caesar": self.caesar_button,
            "keyword": self.keyword_button,
            "affine": self.affine_button,
            "multilateral": self.multilateral_button,
            "vigenere": self.vigenere_button,
            "autokey_ciphertext": self.autokey_ciphertext_button,
            "autokey_plaintext": self.autokey_plaintext_button,
            "playfair": self.playfair_button,
            "permutation": self.permutation_button,
            "column_permutation": self.column_permutation_button,
            "double_transposition": self.double_transposition_button,
            "rc4": self.RC4,
            "ca": self.CA,
            "des": self.DES,
            "aes": self.AES,
        ...
        for name, button in self.cipher_map.items():
            self.button_mapper.setMapping(button, name)

    def setup_connect(self):
        self.button_mapper.mapped[str].connect(self.show_widgets)
        for button in self.cipher_map.values():
            button.toggled["bool"].connect(self.button_mapper.map)

        # Default connections
        buttons_to_reset = [
            self.encrypt_button,
            self.decrypt_button,
            self.encrypt_button_2,
            self.decrypt_button_2,
            self.encrypt_button_3,
            self.decrypt_button_3,
            self.encrypt_button_6,
            self.decrypt_button_6,
            self.encrypt_button_7,
            self.decrypt_button_7,
            self.generate_keypair_button,
            self.generate_keypair_button_2,
        ]
        for button in buttons_to_reset:
            button.clicked.connect(self.default_clicked)
    ...
    def import_file_button_clicked(self):
        self.file_to_encrypt_name, file_to_encrypt_type = QFileDialog.getOpenFileName(
            self,
            "请选择要导入的文件",
            "",
        )
        ...
        try:
            ...
            self.statusbar.showMessage(
                "导入文件" + self.file_to_encrypt_name + "成功", 2000
            )
        except Exception:
    ...
    def path_button_clicked(self):
        self.encrypted_file_to_save_name, file_to_encrypt_type = (
        ...
        self.statusbar.showMessage(
            "加密后的文件将保存为" + self.encrypted_file_to_save_name, 5000
        )
    ...
    def tool_button_3_clicked(self):
        self.file_to_decrypt_name, file_to_encrypt_type = QFileDialog.getOpenFileName(
            self,
            "请选择要导入的文件",
            "",
        )
        ...
        try:
            ...
            self.statusbar.showMessage(
                "导入文件" + self.file_to_decrypt_name + "成功", 2000
            )
        except Exception:
            self.statusbar.showMessage("文件导入失败", 5000)
        finally:
    ...
    def path_button_2_clicked(self):
        self.decrypted_file_to_save_name, file_to_encrypt_type = (
            QFileDialog.getOpenFileName(
                self,
                "请选择要保存到的文件",
                "",
        ...
        self.statusbar.showMessage(
            "解密后的文件将保存为" + self.decrypted_file_to_save_name, 5000
        )
    ...
    def import_plaintext_button_clicked(self):
        self.import_plaintext_button_file_name, import_plaintext_button_file_type = (
            QFileDialog.getOpenFileName(
                self,
                "请选择要导入的文件",
                "",
        ...
        if not self.import_plaintext_button_file_name:
            ...
            try:
                ...
                self.statusbar.showMessage(
                    "已成功从" + self.import_plaintext_button_file_name + "导入文件",
                    2000,
                )
    ...
    def export_ciphertext_button_clicked(self):
        self.export_ciphertext_button_file_name, export_plaintext_button_file_type = (
            QFileDialog.getOpenFileName(
                self,
                "导出至",
                "",
        ...
        if not self.export_ciphertext_button_file_name:
            ...
            try:
                ...
                if not self.cipher_text_edit.toPlainText():
                    ...
                    self.statusbar.showMessage(
                        "已成功将密文写入" + self.export_ciphertext_button_file_name,
                        2000,
                    )
            except Exception:
                self.statusbar.showMessage(
                    "打开或写入" + self.export_ciphertext_button_file_name + "文件失败",
                    5000,
                )
            finally:
                if f:
    ...
    def import_key_from_file(self, target_widget):
        file_name, _ = QFileDialog.getOpenFileName(self, "Import Key File", "")
        if not file_name:
            return
        try:
            with open(file_name, "r") as f:
                ...
                self.statusbar.showMessage(f"Successfully imported key from {file_name}", 2000)
        except (UnicodeDecodeError, IOError) as e:
            self.statusbar.showMessage(f"Failed to open or read {file_name}: {e}", 5000)

    def export_key_to_file(self, source_widget):
        file_name, _ = QFileDialog.getOpenFileName(self, "Export Key File", "")
        if not file_name:
            return
        key_text = source_widget.text()
        if not key_text:
            self.statusbar.showMessage("No key to export", 5000)
            return
        try:
            with open(file_name, "a") as f:
                f.write(key_text)
                self.statusbar.showMessage(f"Successfully exported key to {file_name}", 2000)
        except IOError as e:
            self.statusbar.showMessage(f"Failed to write to {file_name}: {e}", 5000)
    ...
    def export_key_pair(self):
        ...
        if not self.export_private_key_button_file_name:
            ...
            try:
                ...
                if (
                    not self.show_keypair_text.toPlainText()
                    or not self.show_keypair_text_2.toPlainText()
                ):
                    ...
                    if not self.save_path_label.text():
                        try:
                            default_encrypted_file_save_path = (
                                self.import_file_path_edit.text() + ".encrypted"
                            )
                            self.save_path_label.setText(
                                default_encrypted_file_save_path
                            )
                            cipher.encrypt_file(
                                self.import_file_path_edit.text(),
                                default_encrypted_file_save_path,
                            )
                            self.statusbar.showMessage(
                                "加密成功，文件默认保存至"
                        ...
                        try:
                            ...
                            self.statusbar.showMessage(
                                "加密成功，文件已保存至" + self.save_path_label.text(),
                                5000,
                            )
                        except Exception:
                            self.statusbar.showMessage("加密失败")
        ...
        if not self.plain_text_edit_2.toPlainText():
            ...
            if not self.input_key_2.text():
                ...
                if len(self.input_key_2.text()) != 8:
                    ...
                    self.cipher_text_edit_2.setPlainText(
                        aes_string.encrypt(
                            self.plain_text_edit_2.toPlainText(),
                            self.input_key_2.text(),
                        )
                    )
                    self.statusbar.showMessage("加密成功", 2000)
    ...
    def rc4_encrypt_string_button_clicked(self):
        if not self.plain_text_edit_2.toPlainText():
            ...
            if not self.input_key_2.text():
                ...
                self.cipher_text_edit_2.setPlainText(
                    cipher.encrypt(
                        self.input_key_2.text(), self.plain_text_edit_2.toPlainText()
                    )
                )
                self.statusbar.showMessage("加密成功", 2000)
            ...
            if not self.input_key_3.text():
                ...
                if not self.save_path_label.text():
                    ...
                    try:
                        ...
                        self.statusbar.showMessage(
                            "加密成功，文件已保存至" + self.save_path_label.text(), 5000
                        )
                    except Exception:
                        self.statusbar.showMessage("加密失败")
    ...
    def ca_encrypt_string_button_clicked(self):
        if not self.plain_text_edit_2.toPlainText():
            ...
            if not self.input_key_2.text():
                ...
                if (
                    int(self.input_key_2.text()) > 255
                    or int(self.input_key_2.text()) < 0
                ):
        ...
        if not self.cipher_text_edit_2.toPlainText():
            ...
            if not self.input_key_2.text():
                ...
                if len(self.input_key_2.text()) != 8:
                    ...
                    self.plain_text_edit_2.setPlainText(
                        aes_string.decrypt(
                            self.cipher_text_edit_2.toPlainText(),
                            self.input_key_2.text(),
                        )
                    )
                    self.statusbar.showMessage("解密成功", 2000)
    ...
    def rc4_decrypt_string_button_clicked(self):
        if not self.cipher_text_edit_2.toPlainText():
            ...
            if not self.input_key_2.text():
                ...
                self.plain_text_edit_2.setPlainText(
                    cipher.decrypt(
                        self.input_key_2.text(), self.cipher_text_edit_2.toPlainText()
                    )
                )
                self.statusbar.showMessage("解密成功", 2000)
                    ...
                    if not self.lineEdit_4.text():
                        try:
                            default_decrypted_file_save_path = (
                                self.lineEdit_3.text() + ".decrypted"
                            )
                            self.lineEdit_4.setText(default_decrypted_file_save_path)
                            cipher.decrypt_file(
                                self.lineEdit_3.text(), default_decrypted_file_save_path
                            )
                            self.statusbar.showMessage(
                                "解密成功，文件默认保存至"
                                + default_decrypted_file_save_path,
                        ...
                        try:
                            ...
                            self.statusbar.showMessage(
                                "解密成功，文件已保存至" + self.lineEdit_4.text(), 5000
                            )
                        except Exception:
                            self.statusbar.showMessage("解密失败")
            ...
            if not self.show_keypair_text.toPlainText():
                ...
                try:
                    ...
                    self.cipher_text_edit_4.setPlainText(
                        rsa.encrypt(
                            int(e), int(n), self.plain_text_edit_4.toPlainText()
                        )
                    )
                    self.statusbar.showMessage("加密成功", 2000)
                    ...
                    if IOError:
                        self.statusbar.showMessage(
                            "打开或写入" + self.rsa_public_key_file_name + "文件失败",
                            5000,
                        )
        ...
        if not self.import_file_path_edit_3.text():
            ...
            if not self.show_keypair_text_2.toPlainText():
                ...
                if not self.save_path_label_3.text():
                    try:
                        default_encrypted_file_save_path = (
                            self.import_file_path_edit_3.text() + ".encrypted"
                        )
                        ...
                        try:
                            ...
                            rsa.encode_file(
                                int(e),
                                int(n),
                                self.import_file_path_edit_3.text(),
                                default_encrypted_file_save_path,
                            )
                        except (IOError, Exception):
                            if IOError:
                    ...
                    try:
                        ...
                        try:
                            ...
                            if f:
                                f.close()
                        self.statusbar.showMessage(
                            "加密成功，文件已保存至" + self.save_path_label_3.text(),
                            5000,
                        )
                    except Exception:
                        self.statusbar.showMessage("加密失败")
        ...
        if not self.lineEdit_7.text():
            ...
            if not self.show_keypair_text_2.toPlainText():
                ...
                if not self.lineEdit_8.text():
                    try:
                        default_decrypted_file_save_path = (
                            self.lineEdit_7.text() + ".decrypted"
                        )
                    ...
                    try:
                        ...
                        try:
                            ...
                            if f:
                                f.close()
                        self.statusbar.showMessage(
                            "解密成功，文件已保存至" + self.lineEdit_8.text(), 5000
                        )
                    except Exception:
                        self.statusbar.showMessage("解密失败")
            ...
            if not self.show_keypair_text.toPlainText():
                ...
                try:
                    ...
                    self.ecc_result = cipher.encrypt(
                        self.plain_text_edit_4.toPlainText().encode("utf-8")
                    )
                    self.cipher_text_edit_4.setPlainText(self.ecc_result.hex())
                    self.statusbar.showMessage("加密成功", 2000)
                except (IOError, Exception):
                    if IOError:
                        self.statusbar.showMessage(
                ...
                if not self.save_path_label_3.text():
                    try:
                        ...
                        try:
                            ...
                            y = f.readline()
                            with open(
                                self.import_file_path_edit_3.text(), "rb"
                            ) as import_file:
    ...
    def show_widgets(self, button):
        ...
        if button == "md5":
            ...
            if button == "rsa":
                ...
                self.generate_keypair_button.clicked.connect(
                    self.rsa_generate_keypair_button_clicked
                )
                self.generate_keypair_button_2.clicked.connect(
                    self.rsa_generate_keypair_button_clicked
                )
                self.encrypt_button_6.clicked.connect(
                    self.rsa_encrypt_string_button_clicked
                )
                self.decrypt_button_6.clicked.connect(
                    self.rsa_decrypt_string_button_clicked
                )
                self.encrypt_button_7.clicked.connect(
                    self.rsa_encrypt_file_button_clicked
                )
                self.decrypt_button_7.clicked.connect(
            ...
            if button == "rc4":
                ...
                self.encrypt_button_2.clicked.connect(
                    self.rc4_encrypt_string_button_clicked
                )
                self.decrypt_button_2.clicked.connect(
                    self.rc4_decrypt_string_button_clicked
                )
                self.encrypt_button_3.clicked.connect(
                    self.rc4_encrypt_file_button_clicked
                )
                self.decrypt_button_3.clicked.connect(
                ...
                self.input_key_2.setPlaceholderText(
                    "仅限大小写字母和数字的组合，不限位数"
                )
                self.input_key_3.setMaxLength(65535)
                self.input_key_3.setPlaceholderText(
                    "仅限大小写字母和数字的组合，不限位数"
                )
                self.input_key_3.setMaxLength(65535)
        ...
        # Classical Ciphers
            ...
            action_map = {
                "caesar": (self.caesar_encrypt_button_clicked, self.caesar_decrypt_button_clicked, "凯撒密码", "请输入偏移位数，仅限数字，不能超过26", self.classical_validator_2),
                "keyword": (self.keyword_encrypt_button_clicked, self.keyword_decrypt_button_clicked, "关键字密码", "请输入关键字，只能为大小写字母", self.classical_validator_3),
                "affine": (self.affine_encrypt_button_clicked, self.affine_decrypt_button_clicked, "仿射密码", "请输入a和b(仅限数字)，以一个" "(空格)分隔", self.classical_validator_4),
                "multilateral": (self.multilateral_encrypt_button_clicked, self.multilateral_decrypt_button_clicked, "多边密码", "请输入密钥，仅限5位大小写字母", self.classical_validator_3),
                "vigenere": (self.vigenere_encrypt_button_clicked, self.vigenere_decrypt_button_clicked, "维吉尼亚", "请输入密钥，只能为大小写字母", self.classical_validator_3),
                "autokey_ciphertext": (self.autokey_ciphertext_encrypt_button_clicked, self.autokey_ciphertext_decrypt_button_clicked, "Autokey密", "请输入密钥，只能为大小写字母", self.classical_validator_3),
                "autokey_plaintext": (self.autokey_plaintext_encrypt_button_clicked, self.autokey_plaintext_decrypt_button_clicked, "Autokey明", "请输入密钥，只能为大小写字母", self.classical_validator_3),
                "playfair": (self.playfair_encrypt_button_clicked, self.playfair_decrypt_button_clicked, "波雷费密码", "请输入密钥，只能为大小写字母", self.classical_validator_3),
                "permutation": (self.permutation_encrypt_button_clicked, self.permutation_decrypt_button_clicked, "置换密码", "请输入密钥，只能为数字", self.classical_validator_2),
                "column_permutation": (self.column_permutation_encrypt_button_clicked, self.column_permutation_decrypt_button_clicked, "列置换密码", "请输入密钥，只能为大小写字母", self.classical_validator_3),
                "double_transposition": (self.double_transposition_encrypt_button_clicked, self.double_transposition_decrypt_button_clicked, "双重置换", "请输入a和b(仅限大小写字母)，以一个空格分隔", self.classical_validator_5),
            }

            if button in action_map:
                encrypt_action, decrypt_action, label, placeholder, validator = action_map[button]
                self.encrypt_button.clicked.disconnect()
                self.decrypt_button.clicked.disconnect()
                self.encrypt_button.clicked.connect(encrypt_action)
                self.decrypt_button.clicked.connect(decrypt_action)
                self.input_key.setText("")
                self.plain_text_edit.setPlainText("")
                self.cipher_text_edit.setPlainText("")
                self.label.setText(label)
                self.input_key.setPlaceholderText(placeholder)
                self.input_key.setMaxLength(65535 if button not in ["multilateral"] else 5)
                self.input_key.setValidator(validator)
