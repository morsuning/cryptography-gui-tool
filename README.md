...
Project dependencies are listed in `requirements.txt` (PyQt5 and related packages).
...
`uv` is a fast Python package management and execution tool. You can choose to use `uv` to manage virtual environments, install dependencies, and run this project.
...
# Or universal installation script
...
```bash
# Create and activate virtual environment in project root directory
uv venv
source .venv/bin/activate

# Install dependencies (read from requirements.txt)
...
```bash
# Run GUI program directly
uv run python main.py
```

> Note: If dependencies have been installed via `uv pip install -r requirements.txt`, `uv run` will reuse the current virtual environment; you can also use `uv run` without activating the environment to automatically resolve and run.
